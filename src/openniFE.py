...
def openDevice(video_path):
    ...
    try:
        if sys.platform == "win32":
            libpath = "opennife/src/lib/Windows"
        else:
            libpath = "opennife/lib/Linux"
        ...
        with open('timestampsdepth.txt', 'w') as tfile: # write timestamps to txt file
            for i in range(1, nframes+1, interval): # iterate through frames
                ...
                if not os.path.exists(dst16bit):
                    os.mkdir(dst16bit)
                folder_name = os.path.basename(dst) # get the folder name
                cv2.imwrite(dst16bit + "/" + folder_name + "_" + str(frame_depth.frameIndex) +"_16bit.png", depth_array)# save 16 bit image with folder name
                cv2.imwrite(dst8bit + "/" + folder_name + "_" + str(frame_depth.frameIndex)   +"_8bit.png", depth_uint8)# save 8 bit image with folder name
